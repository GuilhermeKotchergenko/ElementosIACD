...
def draw_board(screen, board, bg):
    ...
    for mode in ["<--"]:
        button_rect = pygame.Rect(BUTTON_X // 1.7, button_y + 430, BUTTON_WIDTH // 3.2, BUTTON_HEIGHT)
        pygame.draw.rect(screen, button_color, button_rect, 0, 70)

        #Check if mouse is hovering over the button
        mouseX, mouseY = pygame.mouse.get_pos()
        if button_rect.collidepoint(mouseX, mouseY):
            pygame.draw.rect(screen, button_hover_color, button_rect, 0, 70)
